...
async def preTestSetup(dut, inst_or_path):
    ...
    if isinstance(inst_or_path,Path):
        riscv_binary_utils.loadBinaryIntoMemory(dut,inst_or_path)
    else:
        riscv_binary_utils.asm(dut,inst_or_path)
        pass
    await ClockCycles(dut.clk, 1)
...
def test_runCocotbTestsProcessor(pytestconfig):
    """run processor tests"""

    verilog_sources = [ PROJECT_PATH / "DatapathPipelined.v" ]
    ...
    runr.test(
        ...
        testcase=getattr(pytestconfig.option, "tests", None), # filter tests via the `--tests` command-line flag
    )
    pass

def test_runCocotbTests(pytestconfig):
...
#     "Test fence inst."
#     await preTestSetup(dut,'''
#         li x2,0xfffff0b7 # machine code for `lui x1,0xfffff`. NB: li needs 2 inst. lui+addi sequence
...
RV_TEST_BINARIES = [
    ...
    # self-modifying code and fence.i inst.
